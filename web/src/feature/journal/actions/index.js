...
export function fetchJournal() {
  return (dispatch) => {
    ...
    const ROW_COUNT = 25;
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.First.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Second.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Third.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Fourth.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Fifth.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Sixth.push({ ...media });
    });
    [...Array(ROW_COUNT)].forEach((_, i) => {
      data.Seventh.push({ ...media });
    });
